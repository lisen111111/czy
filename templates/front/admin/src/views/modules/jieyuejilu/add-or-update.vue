<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="到期日期" prop="daoqiriqi">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.daoqiriqi" 
                type="datetime"
                :readonly="ro.daoqiriqi"
                placeholder="到期日期">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.daoqiriqi" label="到期日期" prop="daoqiriqi">
              <el-input v-model="ruleForm.daoqiriqi" 
                placeholder="到期日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  created() {
	this.ruleForm.jieshuriqi = this.getCurDateTime()
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.jieshuriqi = true;
          ...
          if(o=='daoqiriqi'){
            this.ruleForm.daoqiriqi = obj[o];
	    this.ro.daoqiriqi = true;
            continue;
          }
          if(o=='beizhu'){
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jieyuejilu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jieyuejiluCrossAddOrUpdateFlag = false;
						 this.parent.search();
