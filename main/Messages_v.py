...
def messages_resetPass(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        try:
            __loginUserColumn__= messages.__loginUserColumn__
        except:
        ...
        records=messages.getbyparams(messages, messages, recordsParam)
        if len(records)<1:
            msg['code'] = 400
            msg['msg'] = '用户不存在'
            return JsonResponse(msg)

        eval('''messages.objects.filter({}='{}').update({}='{}')'''.format(username_str,username,password_str,init_pwd))
...
def messages_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if  __isAdmin__ == "是":
            ...
            if tablename!="users" and 'messages'[:7]!='discuss'and "userid" in messages.getallcolumn(messages,messages):
                req_dict["userid"] = request.session.get("params").get("id")
        ...
        msg['data']['pageSize']  =messages.page(messages, messages, req_dict, request)
...
def messages_info(request,id_):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if __browseClick__=="是"  and  "clicknum"  in messages.getallcolumn(messages,messages):
            try:
...
def messages_importExcel(request):
    if request.method in ["POST", "GET"]:
        ...
        if file_type in ['xlsx', 'xls']:
            ...
            try:
                for row in range(1, rows):
                    ...
                    req_dict = {}
                    messages.createbyreq(messages, messages, req_dict)
...
def messages_sendemail(request):
    if request.method in ["POST", "GET"]:
        req_dict = request.session.get("req_dict")
...
def messages_autoSort2(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        leixing = set()
        try:
            cursor.execute("select inteltype from storeup where userid = %d"%(request.session.get("params").get("id"))+" and tablename = 'messages' order by addtime desc")
        ...
        L = []
        cursor.execute("select * from messages where $intelRecomColumn in ('%s"%("','").join(leixing)+"') union all select * from messages where $intelRecomColumn not in('%s"%("','").join(leixing)+"')")
...
def messages_value(request, xColumnName, yColumnName, timeStatType):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": {}}
...
def messages_o_value(request, xColumnName, yColumnName):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": {}}
        
        where = ' where 1 = 1 '
        
        sql = "SELECT {0}, sum({1}) AS total FROM messages {2} GROUP BY {0}".format(xColumnName, yColumnName, where)
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
...
def messages_alipay(request):
    if request.method in ["POST", "GET"]:
        ...
        order_string = alipay.api_alipay_trade_page_pay(
            ...
            return_url='http://localhost:8080/zxjyts/messages/notify',
            #notify_url=''
        )
...
def messages_notify(request):
    if request.method in ["POST", "GET"]:
        req_dict = request.session.get("req_dict")
        out_trade_no = req_dict['out_trade_no']
        cursor = connection.cursor()
        
        return redirect('http://localhost:8080/zxjyts/admin/dist/index.html#/messages')
